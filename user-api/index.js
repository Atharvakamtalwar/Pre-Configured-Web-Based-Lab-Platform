...
mongoose
  .connect(
    "mongodb+srv://user-admin:user-admin@customerservicechat.4uk1s.mongodb.net/?retryWrites=true&w=majority&appName=CustomerServiceChat"
  )
...
class AssignmentGeneratorAgent {
  constructor(geminiApiKey) {
    this.apiKey = geminiApiKey || "AIzaSyAsguxgKdBawU1dwrML5rX8nZYbLmwAkDY";
    this.geminiUrl =
...
`;
  ...
  _splitCodeTemplate(codeTemplate) {
    ...
    while (remaining) {
      ...
      if (editableStart > 0) {
        parts.push({
          code: remaining.substring(0, editableStart),
          editable: false,
        });
      }
      ...
      const editableContent = remaining.substring(
        editableStart + 10,
        editableEnd
      );
      parts.push({ code: editableContent, editable: true });
...
app.post("/api/generate-assignment", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "teacher" && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only teachers and admins can generate assignments" });
    }
...
app.get("/api/classes/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (
      ...
      classItem.teacher.toString() !== req.user.id
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
...
app.put("/api/classes/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "teacher") {
      return res
        .status(403)
        .json({ message: "Only teachers can update classes" });
    }
...
app.delete("/api/classes/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "teacher") {
      return res
        .status(403)
        .json({ message: "Only teachers can delete classes" });
    }
...
app.post("/api/batches", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "teacher") {
      return res
        .status(403)
        .json({ message: "Only teachers can create batches" });
    }
    ...
    if (!name || !classId) {
      return res
        .status(400)
        .json({ message: "Batch name and class ID are required" });
    }
...
app.put("/api/batches/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "teacher") {
      return res
        .status(403)
        .json({ message: "Only teachers can update batches" });
    }
...
app.delete("/api/batches/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "teacher") {
      return res
        .status(403)
        .json({ message: "Only teachers can delete batches" });
    }
...
app.post("/api/batches/enroll", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "student") {
      return res
        .status(403)
        .json({ message: "Only students can enroll in batches" });
    }
    ...
    if (batch.students.includes(req.user.id)) {
      return res
        .status(400)
        .json({ message: "You are already enrolled in this batch" });
    }
...
app.get("/api/batches/enrolled", authenticateToken, async (req, res) => {
  try {
    ...
    const batches = await Batch.find({ students: req.user.id }).populate({
      path: "class",
...
app.post("/api/assignments", authenticateToken, async (req, res) => {
  try {
    ...
    const { title, description, classId, modules, language, requirements } =
      req.body;
    ...
    if (!title || !description || !classId || !modules || !language) {
      return res
        ...
        .json({
          message:
            "Title, description, class ID, modules, and language are required",
        });
    }
    ...
    const studentAssignmentPromises = Array.from(allStudentIds).map(
      (studentId) => {
        const newStudentAssignment = new StudentAssignment({
          student: studentId,
  ...
  try {
    ...
    if (req.user.role === "teacher") {
      ...
      const stats = {
        ...
        submitted: studentAssignments.filter((sa) => sa.status === "completed")
          .length,
        inProgress: studentAssignments.filter(
          (sa) => sa.status === "in-progress"
        ).length,
        notStarted: studentAssignments.filter((sa) => sa.status === "assigned")
          .length,
...
app.post("/api/assignments/:id/submit", authenticateToken, async (req, res) => {
  try {
    ...
    if (!moduleId || !code) {
      return res
        .status(400)
        .json({ message: "Module ID and code are required" });
    }
    ...
    if (!studentAssignment) {
      return res
        .status(404)
        .json({ message: "Assignment not found or not assigned to you" });
    }

    // Update or add the submission
    const submissionIndex = studentAssignment.submissions.findIndex(
    ...
    studentAssignment.progress = Math.floor(
...
app.post(
  "/api/assignments/:id/modules/:moduleId/complete",
  authenticateToken,
  async (req, res) => {
    try {
      ...
      if (req.user.role !== "student") {
        return res
          .status(403)
          .json({ message: "Only students can mark modules as completed" });
      }

      const assignmentId = req.params.id;
      const moduleId = Number(req.params.moduleId);

      console.log("Looking for student assignment:", {
        student: req.user.id,
        assignment: assignmentId,
      });
      ...
      console.log(
        "Found student assignment:",
        studentAssignment ? studentAssignment._id : "none"
      );
      ...
      const completedModuleIds = studentAssignment.submissions.map(
        (sub) => sub.moduleId
      );
...
app.get(
  "/api/assignments/:id/completed-modules",
  authenticateToken,
  async (req, res) => {
    try {
      console.log(
        "API: Fetching completed modules for assignment:",
        req.params.id
      );
      if (req.user.role !== "student") {
        return res.status(403).json({ message: "Access denied" });
      ...
      const completedModules = studentAssignment.submissions.map(
        (submission) => submission.moduleId
      );
...
app.post("/api/batches/:id/leave", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== "student") {
      return res
        .status(403)
        .json({ message: "Only students can leave batches" });
    }
    ...
    if (!batch.students.includes(req.user.id)) {
      return res
        .status(400)
        .json({ message: "You are not enrolled in this batch" });
    }
  ...
  try {
    ...
    if (req.user.role !== "teacher") {
      return res
        .status(403)
        .json({
          message: "Access denied. Only teachers can access these statistics.",
        });
    }
    ...
    const pendingReviews = studentAssignments.filter(
